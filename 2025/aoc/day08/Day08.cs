...
public class Day08
{
    ...
    {
        var edges = CalculateEdges(points).OrderBy(e => e.Length).ToList();

        return CalculateCircuits2(points, edges).ToString(CultureInfo.InvariantCulture);
    }

    private static double CalculateCircuits2(List<Point> points, List<Edge> edges)
    {
        List<Circuit> circuits = [];
        foreach (var edge in edges)
        {
            ...
            var start = points.SingleOrDefault(p => p == edge.Start);
            var end = points.SingleOrDefault(p => p == edge.End);
            ...
            if (points.Count == 0 && circuits.Count == 1)
            {
